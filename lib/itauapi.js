  ...
  getMonth(hash, month, year) {
    ...
    if (month === (new Date()).getMonth() + 1) {
      movementsPromise = this._rq.post(
        ...
        const rawData = JSON.parse(res.body).itaulink_msg.data;
        return rawData.movimientosMesActual.movimientos;
      });
    } else {
      movementsPromise = this._rq.post(
        ...
        const rawData = JSON.parse(res.body).itaulink_msg.data;
        return rawData.mapaHistoricos.movimientosHistoricos.movimientos;
      });
    }
