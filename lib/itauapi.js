...
module.exports = class ItauAPI {
  ...
  /**
   * Returns the transaction on the given month
   */
  getMonth(hash, month, year) {
    let req;
    if (month === (new Date()).getMonth() + 1) {
      req = this._rq.post(`/cuentas/1/${hash}/mesActual`);
    } else {
      req = this._rq.post(`/cuentas/1/${hash}/${month}/${year}/consultaHistorica`);
    }
    // TODO: Parse
    return req;
  }
  ...
  _downloadAccounts() {
    ...
    return this._rq.get('/').then((res) => {
      ...
      const matchedLine = lines.find(line => line.includes('var mensajeUsuario = JSON.parse'));
